...
public class PIRBodySubcompilerTest {
	...
	public void setUp() {
		...
		myRecurser = myMocksControl.createMock(IPIRBodySubcompiler.class);
		...
		myRegisterStrategy = myMocksControl.createMock(IRegisterStrategy.class);
		myEnvironment = myMocksControl.createMock(IEnvironment.class);
		myTarget = myMocksControl.createMock(IRegister.class);

		myVisitor = new PIRBodySubcompiler(myRecurser, myResourceAllocator,
	...
	public void shouldBindNothingToNewEnvironment() {
		...
		IPIRDeclarationCompiler binder = myMocksControl
				.createMock(IPIRDeclarationCompiler.class);
	...
	public void shouldCompileOperator() {
		...
		IRegister[] registers = new IRegister[] {
				myMocksControl.createMock(IRegister.class),
				myMocksControl.createMock(IRegister.class) };
		ICode operatorCode = myMocksControl.createMock(ICode.class);

		EasyMock.expect(
				myRegisterStrategy.operatorRegisters(myTarget, left, right))
				.andReturn(registers);
		EasyMock.expect(myRecurser.recurse(left, registers[0])).andReturn(
				leftCode);
		EasyMock.expect(code.append(leftCode)).andReturn(code);
		EasyMock.expect(myRecurser.recurse(right, registers[1])).andReturn(
				rightCode);
		...
		EasyMock.expect(
				operatorInstruction.compile(myResourceAllocator, myTarget,
						registers)).andReturn(operatorCode);
	...
	public void shouldCompileLet() {
		...
		IEnvironment<IRegister> newEnvironment = myMocksControl
				.createMock(IEnvironment.class);
